...
const PlaceHolderState = () => {
    ...
    const onInputChange = useCallback((nodeId, inputValue) => {
      ...
      const onFileChange = useCallback((nodeId, selectedFiles) => {
        setNodes((prevNodes) =>
          prevNodes.map((node) =>
            node.id === nodeId ? { ...node, data: { ...node.data, files: selectedFiles.map(file => file.name) } } : node
          )
        );
    }, [setNodes]);
      ...
      const nodeTypes = useMemo(() => ({
        stateNode: (props) => <StateNode {...props} onInputChange={onInputChange} onFileChange={onFileChange} onCheckChange={onCheckChange} onQqCheckChange={onQqCheckChange}/>,
        disableStateNode: DisableStateNode,
        customNode : (props) => <FileSelectorNode {...props} onFileSelect={onFileChange}/>,
        // ... other node types if needed ...
    }), [onInputChange, onFileChange, onCheckChange, onQqCheckChange]); // Adjusted for the updated onFileChange
      ...
      const saveFlowToServer = async () => {
        try {
          ...
          const response = await fetch(server_address+'save_state_flow/' + item, {
            ...
            body: JSON.stringify({ nodes, edges, clientId }),
          });
      ...
      const handleDownload = async () => {
        try {
          ...
          // });
          const response = await fetch(server_address + 'download-bot-json/', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          // First, convert the response to a blob
          ...
          document.body.appendChild(link); // Append the link to the body to ensure it's in the DOM before clicking
          link.click();
          
          // Cleanup: remove the link element
          document.body.removeChild(link); // Ensure the link is removed correctly          
        } catch (error) {
